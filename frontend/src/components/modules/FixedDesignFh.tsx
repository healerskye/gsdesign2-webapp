...
export function FixedDesignFh({ onResult }: Props) {
  ...
  const run = async () => {
    setLoading(true);
    try {
      const result = await api.fixedDesignFh({
  ...
  return (
    <div className="flex flex-col gap-4">
      <h2 className="text-lg font-semibold text-slate-800">Fixed Design - Fleming-Harrington</h2>
      <p className="text-xs text-slate-500">Fleming-Harrington weighted log-rank test for fixed design.</p>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide col-span-2 mt-2">FH Weight Parameters</p>
        <InputField label="Rho" name="rho" value={rho} onChange={setRho} step={0.5} min={0} helpText="FH(rho, gamma) weight" />
        <InputField label="Gamma" name="gamma" value={gamma} onChange={setGamma} step={0.5} min={0} helpText="FH(rho, gamma) weight" />
      </div>
